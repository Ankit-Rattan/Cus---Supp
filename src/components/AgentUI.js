...
const AgentAvatar = ({ size = 120, color = '#E31837' }) => (
  <svg width={size} height={size} viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="60" cy="60" r="58" fill="url(#avatarGrad)" stroke={color} strokeWidth="3"/>
    ...
    <path d="M50 85 L60 100 L70 85" fill={color}/>
    <defs>
...
export default function AgentUI({ 
  ...
  // Placeholder functions - will be connected to ElevenLabs API externally
  async function sendText() {
    ...
    // Simulated response - replace with actual ElevenLabs integration
    setTimeout(() => {
      setMessages((prev) => [...prev, { 
        role: 'agent', 
        text: `Thank you for your message. Our ${config.agentRole} will assist you shortly. This is a demo response - connect to ElevenLabs API for real conversations.`
      }]);
    }, 1000);
  }
  ...
  async function startCall() {
    ...
    // ElevenLabs voice API will be integrated externally
    try {
        ...
        <main style={styles.card}>
          ...
          <div style={styles.modeSwitcher}>
            <button
              style={{ 
                ...styles.modeBtn, 
                ...(mode === 'chat' ? { ...styles.modeBtnActive, background: config.gradient } : {}) 
              }}
            ...
            <button
              style={{ 
                ...styles.modeBtn, 
                ...(mode === 'call' ? { ...styles.modeBtnActive, background: config.gradient } : {}) 
              }}
              ...
              {!isCallActive ? (
                <div style={styles.preCallScreen}>
                  <div style={styles.avatarWrapper}>
                    <AgentAvatar size={130} color={config.color} />
                  </div>
                  <h2 style={styles.title}>Voice {config.agentRole}</h2>
                  <p style={styles.subtitle}>Talk to our {config.agentRole.toLowerCase()} directly</p>
            ...
            <>
              ...
              {messages.length === 0 ? (
                <div style={styles.welcomeSection}>
                  <div style={styles.avatarWrapper}>
                    <AgentAvatar size={110} color={config.color} />
                  </div>
                ...
                <div style={styles.chat}>
                  {messages.map((m, i) => (
                    <div key={i} style={{ ...styles.messageRow, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                      ...
                      <div style={m.role === 'user' ? { ...styles.userBubble, background: config.gradient } : styles.agentBubble}>{m.text}</div>
                    </div>
                  ))}
                  <div ref={chatEndRef} />
                </div>
              ...
              <div style={styles.inputBar}>
                ...
                />
                <button 
                  style={{ ...styles.sendBtn, background: config.gradient, opacity: text.trim() ? 1 : 0.5 }} 
                  onClick={sendText} 
                  disabled={!text.trim()}
                >
                  <SendIcon />
...
const styles = {
  ...
  gradientOverlay: {
    ...
    inset: 0,
    zIndex: 1,
    background: `linear-gradient(135deg, rgba(0, 100, 145, 0.97) 0%, rgba(0, 85, 125, 0.98) 50%, rgba(0, 65, 100, 0.99) 100%)`,
  },
  ...
  card: {
    ...
    flexDirection: 'column',
    overflow: 'hidden',
    minHeight: 'clamp(480px, 62vh, 560px)',
